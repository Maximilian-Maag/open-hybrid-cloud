...
test-db:
	@for db in infrashelf_test infrashelf_e2e; do \
	  docker exec infrashelf-postgres psql -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname='$$db'" | grep -q 1 \
	    && echo "  exists  $$db" \
	    || { docker exec infrashelf-postgres createdb -U postgres "$$db" && echo "  created $$db"; }; \
	done
...
test-db-prune:
	@dbs="$$(docker exec infrashelf-postgres psql -U postgres -tAc \
	...
	for db in $$dbs; do \
	  [ -n "$$db" ] || continue; \
	  docker exec infrashelf-postgres dropdb -U postgres --if-exists "$$db" && echo "  dropped $$db"; \
	done

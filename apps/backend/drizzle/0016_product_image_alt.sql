...
-- point and lets the constraint below be enforced for everything new.
UPDATE "products" p
SET "image_alt" = COALESCE(
  (SELECT t."name" FROM "product_translations" t
    WHERE t."product_id" = p."id" AND t."language_code" = 'en' LIMIT 1),
  (SELECT t."name" FROM "product_translations" t WHERE t."product_id" = p."id" LIMIT 1)
)
WHERE p."image" IS NOT NULL AND p."image_alt" IS NULL;

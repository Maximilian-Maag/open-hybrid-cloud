...
const makeReq = (productId: string, method = 'GET', body?: unknown, auth?: string) =>
  new NextRequest(`http://localhost/api/admin/products/${productId}/environments`, {
    method,
...
describe('GET /api/admin/products/[id]/environments', () => {
  ...
  it('returns environments for root', async () => {
...
describe('POST /api/admin/products/[id]/environments', () => {
  it('returns 401 without auth', async () => {
    const res = await POST(makeReq('1', 'POST', { environmentId: 1, price: '10.00' }), { params: Promise.resolve({ id: '1' }) })
    expect(res.status).toBe(401)
  })
  ...
  it('returns 403 for admin role', async () => {
    const admin = await createUser({ role: 'admin' })
    const auth = await makeAuthHeader(admin)
    const res = await POST(makeReq('1', 'POST', { environmentId: 1, price: '10.00' }, auth), { params: Promise.resolve({ id: '1' }) })
    expect(res.status).toBe(403)
  })

  it('creates product environment for root', async () => {
    const root = await createUser({ role: 'root' })
    const cat = await createCategory()
    const p = await createProduct(cat.id)
    ...
    const res = await POST(
      makeReq(String(p.id), 'POST', { environmentId: env.id, price: '99.00', currency: 'EUR' }, auth),

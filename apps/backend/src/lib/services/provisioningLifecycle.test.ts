...
describe('terminal webhook effects and the point of no return (issue #136)', () => {
  ...
  const blockOutputWrites = async (): Promise<() => Promise<void>> => {
    await db.execute(sql`
      DROP TRIGGER IF EXISTS infrashelf_test_block_outputs ON infrastructure_elements;
      CREATE OR REPLACE FUNCTION infrashelf_test_block_outputs() RETURNS trigger AS $$
      BEGIN RAISE EXCEPTION 'simulated database outage'; END;
      $$ LANGUAGE plpgsql;
      CREATE TRIGGER infrashelf_test_block_outputs
        BEFORE UPDATE OF outputs ON infrastructure_elements
        FOR EACH ROW EXECUTE FUNCTION infrashelf_test_block_outputs();
    `)
    return async () => {
      await db.execute(sql`
        DROP TRIGGER IF EXISTS infrashelf_test_block_outputs ON infrastructure_elements;
        DROP FUNCTION IF EXISTS infrashelf_test_block_outputs();
      `)
    }
  }

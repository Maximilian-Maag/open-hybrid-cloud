...
export function LanguageSwitcher({ currentLang }: Props) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1 text-white/80 hover:text-white text-xs font-medium border border-white/20 rounded-md px-2 py-1 transition-colors focus:outline-none"
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 bg-white rounded-xl shadow-xl border border-slate-200 p-3 w-72">
            <div className="grid grid-cols-3 gap-1">
              {SUPPORTED_LANGUAGES.map((l) => (
                <button
                  key={l.code}
                  onClick={() => selectLang(l.code)}
                  className="w-full flex flex-col items-center rounded-lg px-1 py-2 transition-colors text-center"

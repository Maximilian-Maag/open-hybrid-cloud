...
const navItems: NavItem[] = [
  ...
  { href: '/admin', label: 'Admin', icon: <AdminIcon />, minRole: 'root' },
]
...
export function Sidebar({ role }: SidebarProps) {
  ...
  return (
    <aside className="flex h-full w-60 flex-col bg-slate-900 text-white">
      <div className="flex h-16 items-center px-6 border-b border-slate-700/60">
        <span className="text-lg font-bold tracking-tight text-white">Open Hybrid Cloud</span>
      </div>
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
        {navItems
          .filter((item) => !item.minRole || hasRole(role, item.minRole))
          .map((item) => {

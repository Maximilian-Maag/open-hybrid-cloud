...
describe('clearServiceWorkerCaches', () => {
  it('deletes every cache, not only the shell', async () => {
    const deleted = stubCaches(['infrashelf-shell-v1', 'infrashelf-assets-v1', 'something-else'])
    stubWorker()
    ...
    expect(deleted.sort()).toEqual(['infrashelf-assets-v1', 'infrashelf-shell-v1', 'something-else'])
  })
  ...
  it('also tells the worker, which may hold caches this page cannot enumerate', async () => {
    ...
    expect(postMessage).toHaveBeenCalledWith({ type: 'infrashelf-signout' })
  })
  ...
  it('does not throw when there is no service worker at all', async () => {
    stubCaches(['infrashelf-shell-v1'])
    vi.stubGlobal('navigator', {})

...
\emph{Local} administrator accounts — those signing in with a password rather than
through Entra ID — hold enough privilege to change what the portal does, and are
the accounts no identity provider protects. They are given a second factor:
...
Entra ID account promoted to Admin or Root has no local password:
...
route and refused the one screen that could lift the refusal. Its MFA is the
identity provider's.

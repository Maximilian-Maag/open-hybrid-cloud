...
\paragraph{The internal routes.} These three have no user. A scheduler has no
...
anonymous caller. \code{X-Sweep-Secret} carries
\env{DECOMMISSION\_SWEEP\_SECRET}; \code{X-Drift-Secret} carries
\env{DRIFT\_REPORT\_SECRET} and authorises both drift routes, so a deployment

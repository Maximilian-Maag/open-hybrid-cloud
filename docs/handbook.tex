...
\noindent
The one exception inside that range is \code{/api/auth/callback}, Entra ID's
OAuth callback, which is the \emph{backend's} and must be matched exactly so it
takes priority over the \code{/api/auth} prefix. Everything else under
\code{/api} goes to port 3001, and every other path to port 3000.
...
\code{/api/login-challenge} and \code{/api/ping}, which go to the frontend
(see section on the two-container split). \code{/api/auth/callback} is the
exception to that exception: it is Entra ID's callback, it belongs to the
backend, and it is matched exactly so that it outranks the \code{/api/auth}
prefix.
...
\noindent
with one exception inside that range: \code{/api/auth/callback} is Entra ID's
OAuth callback and belongs to the \emph{backend}, so it is matched exactly and
outranks the \code{/api/auth} prefix. Everything else under \code{/api} goes to
port 3001.
...
\section{Two-Factor Authentication for the Root Account}
\label{sec:root-2fa}

The Root account signs in with a local password and holds the highest privilege
level in the system, so it is the one account no identity provider protects.
It can be given a second factor: standard TOTP (RFC~6238, SHA-1, 30-second
window, six digits), compatible with Google Authenticator, Authy, Bitwarden and
1Password. It is scoped to Root: every two-factor endpoint answers 403 for any
other role, enforced in the service layer rather than per route. Admin and
Project Manager accounts authenticate through Microsoft Entra ID and are covered
by its MFA instead.

...
Administrator accounts sign in with a local password and hold enough privilege to
change what the portal does, so they are the accounts no identity provider
protects. They are given a second factor: standard TOTP (RFC~6238, SHA-1,
30-second window, six digits), compatible with Google Authenticator, Authy,
...
Project Manager accounts are excluded — it is the end-user role, and every
two-factor endpoint answers 403 for it, enforced in the service layer rather than
per route. Accounts that authenticate through Microsoft Entra ID are covered by
its MFA instead.

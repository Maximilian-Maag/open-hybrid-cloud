% ============================================================
%  InfraShelf Infrastructure Webshop — Technical Handbook
% ============================================================
...
\definecolor{isf-blue}{HTML}{131921}
\definecolor{isf-amber}{HTML}{febd69}
...
\usepackage[
  ...
  pdfauthor={InfraShelf Infrastructure Team},
  pdftitle={InfraShelf Infrastructure Webshop — Technical Handbook},
  pdfsubject={Technical documentation for administrators, developers, and project managers},
  pdfkeywords={InfraShelf, webshop, infrastructure, Next.js, TypeScript, OpenTofu, GitLab, C4},
...
\titleformat{\chapter}[display]
  {\normalfont\huge\bfseries\color{isf-blue}}
  {\chaptertitlename\ \thechapter}{12pt}{\Huge}
\titleformat{\section}
  {\normalfont\Large\bfseries\color{isf-blue!80}}
  {\thesection}{1em}{}
\titleformat{\subsection}
  {\normalfont\large\bfseries\color{isf-blue!60}}
  {\thesubsection}{1em}{}
...
% A path through the UI, as the reader would click it: Admin -> Environments.
% Distinct from \code, which is for things typed rather than clicked.
...
\begin{titlepage}
  \begin{tikzpicture}[remember picture, overlay]
    \fill[isf-blue] (current page.north west) rectangle
      ([yshift=-5cm] current page.north east);
    \fill[isf-amber] ([yshift=-5cm] current page.north west) rectangle
  ...
  \vspace{1cm}
  {\small\color{gray}InfraShelf — Digital Unit Infrastructure Team}
\end{titlepage}
...
InfraShelf is an open-source, self-service portal that enables Admins and Project Managers to order, manage, and decommission IT infrastructure on Linode/Akamai Cloud through a browser-based interface. The source code is publicly available at \url{https://github.com/Maximilian-Maag/infrashelf}. This handbook is the authoritative reference for everyone who operates, extends, or uses the system.
...
\begin{enumerate}
  ...
  \item The service claims the order (returns \code{Err 400} if it is not
        \code{pending}). Where the environment sets
        \code{respectsDeploymentWindows} and no window is open, the path stops
        here: the order becomes \code{scheduled} carrying the instant its window
        opens, the approval succeeds, and provisioning waits for the sweep or for
        root's \menu{Deploy now} (\S\ref{sec:deployment-windows}). Otherwise it
        continues straight on, and calls \code{triggerProductWebhooks()} and
...
with InfraShelf's open-source software policy.
...
\begin{tikzpicture}[node distance=1.2cm and 1.6cm,
  box/.style={rectangle, draw=isf-blue!70, fill=isf-blue!10,
  ...
  \node[box, right=1.6cm of nginx] (app) {infrashelf\\\scriptsize Docker\\:8080};
...
\begin{lstlisting}[style=bash]
git clone https://github.com/Maximilian-Maag/infrashelf.git
cd infrashelf
\end{lstlisting}
...
\begin{lstlisting}[style=bash]
# Backup PostgreSQL
docker exec isf-postgres pg_dump -U postgres infrashelf \
  | gzip > backup-$(date +%Y%m%d).sql.gz

# Restore
zcat backup-20260101.sql.gz \
  | docker exec -i isf-postgres psql -U postgres infrashelf
\end{lstlisting}
...
\begin{tikzpicture}[node distance=1.2cm and 1.8cm,
  ...
  svc/.style={rectangle, rounded corners=4pt, draw=isf-amber!80, fill=isf-amber!20,
...
\begin{itemize}
  ...
  \item The images \code{maximilianmaag/infrashelf-backend} and
        \code{maximilianmaag/infrashelf-frontend} are publicly
        available on Docker Hub — no \code{imagePullSecret} required
\end{itemize}

\subsection*{Deployment with Helm}

The repository ships a Helm chart at \file{infra/helm/infrashelf/}.
...
helm upgrade --install infrashelf \
  infra/helm/infrashelf \
  --namespace infrashelf \
  --create-namespace \
  --set backend.env.databaseUrl="postgres://user:pass@pg.example.com:5432/infrashelf?sslmode=require" \
  ...
  --set frontend.env.apiUrl="http://infrashelf-backend:3001" \
  --set ingress.hosts[0].host=shop.example.com \
  --set ingress.tls[0].hosts[0]=shop.example.com \
  --set ingress.tls[0].secretName=infrashelf-tls
\end{lstlisting}
...
backend:
  env:
    databaseUrl: "postgres://user:pass@pg.example.com:5432/infrashelf?sslmode=require"
...
\begin{lstlisting}[style=bash]
helm upgrade --install infrashelf infra/helm/infrashelf \
  --namespace infrashelf --create-namespace \
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}Xl@{}}
    ...
    \midrule
    \code{backend.image.repository}       & Backend Docker Hub image          & \code{maximilianmaag/infrashelf-backend} \\
    ...
    \code{frontend.image.repository}      & Frontend Docker Hub image         & \code{maximilianmaag/infrashelf-frontend} \\
...
\begin{lstlisting}[style=bash]
kubectl create namespace infrashelf
\end{lstlisting}

\item \textbf{(Optional) Create an image pull secret.}

The container image \code{maximilianmaag/infrashelf} is publicly
...
kind: Secret
metadata:
  name: isf-backend-env
  namespace: infrashelf
type: Opaque
stringData:
  DATABASE_URL: "postgres://user:password@pg.example.com:5432/infrashelf?sslmode=require"
...
\begin{lstlisting}[style=bash]
kubectl exec -n infrashelf deploy/isf-backend -- \
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: app
          image: maximilianmaag/infrashelf:latest
          ports:
            - containerPort: 8080
          envFrom:
            - secretRef:
...
kubectl rollout status deployment/infrashelf -n infrashelf
kubectl get ingress -n infrashelf
\end{lstlisting}
...
# Kubernetes
kubectl exec -n infrashelf deploy/isf-postgres -- \
  psql -U postgres -d infrashelf -c \
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}Xll@{}}
    ...
    Root    & Catalog \& config; everything an admin can do & Local & \code{root} \\
    Project Manager  & Orders own infra; approval required     & Local & \code{project\_leader} \\
    \bottomrule
  \end{tabularx}
\end{table}

\noindent This table said ``Root: no ordering'' until \#363, and that was
simply false --- \code{POST /api/cart/checkout} and \code{POST /api/orders}
call \code{requireAuth}, not \code{requireRole}, so root has always been able
to order. Worth naming, because a capability table that is wrong is worse than
no table: it is the thing somebody checks instead of reading the code.

\subsection*{What each role is responsible for}
\label{sec:role-responsibilities}

The table above is about \emph{permission}. This one is about \emph{purpose} —
the journey each role exists to complete, which is a different question: an
endpoint can be reachable and the journey through it still broken at a handover.

\begin{table}[H]
  \centering
  \caption{Role responsibilities, and where each is proven}
  \begin{tabularx}{\linewidth}{@{}l>{\raggedright\arraybackslash}X@{}}
    \toprule
    \textbf{Role} & \textbf{Responsible for} \\
    \midrule
    Project Manager & Placing orders for their own projects; managing those
      projects and their members; seeing their own orders and infrastructure.
      Approves nothing --- not even their own order. \\
    Admin & Approving or rejecting orders, including under a delegation;
      ordering directly, with no approval step; seeing every order and every
      infrastructure element; decommissioning. \\
    Root & The catalogue, environments, CI sources, users, cost centres,
      branding and configuration; deployment windows and the deploy-now
      override (\S\ref{sec:deployment-windows}); everything an admin can do. \\
    \bottomrule
  \end{tabularx}
\end{table}

\noindent \file{e2e/role-journeys.spec.ts} is the executable half. It plays the
handovers as separate sessions with real sign-outs between them --- a project
manager orders and goes home, an admin decides the next morning, the project
manager signs back in and sees the answer --- because two people signed in at
once in one browser is not what happens, and a suite that only ever tests it
that way cannot see a session that fails to survive the round trip. It did not
see \#359, where signing out did nothing at all.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lp{9cm}@{}}
    ...
    \code{cost\_centers}            & Billing cost centres referenced by projects and orders; optionally carry a budget (amount, currency, period, behaviour --- all four together or none, enforced by CHECK) \\
...
\centering
\begin{tikzpicture}[
  tbl/.style={rectangle, draw=isf-blue!50, fill=isf-blue!6,
  ...
  casc/.style={->, >=Stealth, isf-blue!80, thick, dashed},
...
import type { Widget } from '@infrashelf/types'
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  state/.style={rectangle, rounded corners=6pt, draw=isf-blue!60,
                fill=isf-blue!10, minimum width=3.2cm,
  ...
  % #330. Above the row rather than below it, so the ordinary path stays a
  % straight line: this is a detour an approved order takes, not a failure.
  \node[state, above=0.8cm of prov] (sched) {Scheduled};
  ...
  \draw[arr] (pend)   -- node[c4label,above left]{\scriptsize approved,\\outside a window} (sched);
  \draw[arr] (sched)  -- node[c4label,right]{\scriptsize window opens,\\or root deploys now} (prov);
...
\section{Deployment Windows}
\label{sec:deployment-windows}

An approved order does not always provision immediately. Where an environment
opts in, provisioning waits until a \emph{deployment window} is open --- so that
deployments happen while admins are present to watch them, rather than at
22:00 on a Friday.

\paragraph{What root configures.} Under \menu{Admin $\rightarrow$ Deployment
windows}: a time zone, and any number of windows given as a start time and a
duration. The same pattern applies every working day; there is no per-weekday
editing, because the question the feature answers is ``when is somebody
watching'', and that is a property of the working day rather than of Tuesday.

Windows are validated as a set --- no overlaps, none crossing midnight, each at
least a minute long --- and saved as a set. That is deliberate: an overlap is a
property of the collection, so neither of two overlapping windows is wrong on
its own, and moving 08:00 to 13:00 while 13:00 still exists has to be one save
rather than an illegal intermediate state.

Saturdays and Sundays are excluded by the weekday arithmetic. Public holidays
come from the \code{holidays} table, which is the subject of the rest of this
section.

\paragraph{Where the holidays come from.} Root gives a feed URL --- an ICS
calendar or a JSON list, both shapes are read --- and the resolved dates are
\emph{cached in the database}. Deciding whether an order may deploy therefore
never makes a network call, which is the whole point: an order must not wait on
somebody else's web server.

The cost of that trade is that stale data looks exactly like fresh data, so the
admin screen says how old the answer is and complains past thirty days. A
refresh that fails changes nothing: the last good set stays, and the failure is
recorded rather than replacing the dates with none --- which would turn ``the
feed is down'' into ``no day is a holiday''.

\paragraph{Two things no feed knows.} A company shutdown week that no public
calendar lists, and a public holiday this company works through. Both are added
or amended by hand and stamped \code{manual}, and a refresh leaves those rows
alone --- otherwise the nightly job would quietly undo the decision every night.
Un-observing a date takes the row over rather than deleting it, because a
deleted \code{feed} row simply comes back on the next refresh.

\paragraph{Fail closed, and which way that points.} If a feed is configured and
has \emph{never} been read successfully, the portal has no holiday data at all
--- a public holiday and a working day are indistinguishable. Applying the
windows then would mean provisioning on Christmas morning while claiming the
opposite, so the windows are \textbf{not applied}: orders provision immediately,
exactly as they did before the feature existed, and the admin screen says so in
red.

That is deliberately not the other direction. Holding every order until somebody
fixes a feed would turn a third party's outage into a queue nobody asked for and
the requester would have no idea. A feed that worked once and is failing now
does \emph{not} trigger this: it has a last good answer, and public holidays do
not move often.

\paragraph{Refreshing it.} \code{POST /api/internal/holiday-refresh}, with
\env{DEPLOYMENT\_WINDOW\_SWEEP\_SECRET} in \code{X-Sweep-Secret} --- the same
secret as the sweep, because it is the same feature run by the same operator.
The Helm chart ships a daily CronJob alongside the sweep's; daily is ample, as
public holidays are published a year ahead. Root can also refresh from the admin
screen, and preview a feed's dates before saving the URL.

\paragraph{The second switch.} A window set alone changes nothing. Each
environment carries \code{respectsDeploymentWindows}, \textbf{default off},
edited on the environment itself under \menu{Admin $\rightarrow$ Environments}.
Production honours the windows; a sandbox that stops deploying after 18:00 is an
obstacle rather than a safeguard. The default matters on upgrade: turning this on
globally would strand every out-of-hours deploy in the portal at once.

\paragraph{What an order does.} The approval itself never fails for being out of
hours --- the admin's decision is recorded either way and the work is not thrown
away. Only the provisioning waits, in the \code{scheduled} state, carrying the
instant its window opens. The order page shows that instant rather than only the
badge.

\paragraph{Releasing them.} \code{POST /api/internal/deployment-window-sweep},
authenticated with \env{DEPLOYMENT\_WINDOW\_SWEEP\_SECRET} in
\code{X-Sweep-Secret}, releases every order whose window has opened. The backend
has no worker process and is horizontally scaled, so an in-process timer would
fire once per replica; an external scheduler is the single caller instead --- the
Helm chart ships a CronJob (\code{deploymentWindowSweep.enabled}), and a Docker
host wants a cron entry.

Call it \emph{often}, every minute or two. That interval is the delay between a
window opening and the orders waiting on it actually deploying, and an admin who
came in to watch the 08:00 deployment should not be watching an empty screen at
08:14.

\paragraph{Deploying early.} Root, and only root, can release a scheduled order
without waiting, with \menu{Deploy now} on the order. An admin approving decides
that an order should happen; this decides it should happen outside the hours the
company said it watches its systems, so it is root's call --- audited as
\code{order.window\_overridden}, naming who and which window was skipped. The
override is recorded before provisioning is attempted: the decision was made
whether or not CI then answered.
...
\textbf{Parameter hierarchy}. A name can be defined at more than one
level, and the order form resolves it to exactly ONE effective
definition --- the one a submitted value is validated against. The rules,
in order:

\begin{enumerate}
  \item \textbf{Scope}: product beats category beats global.
  \item \textbf{Environment}: at the same scope, a definition for one
        environment beats one that applies to all environments.
  \item \textbf{Project narrowing}: at the same scope and environment, a
        definition narrowed to specific projects beats an unnarrowed one,
        for those projects.
  \item \textbf{Most recent}: if two definitions are alike on all three,
        the one created last wins.
\end{enumerate}

Rule 4 exists because nothing prevents two parameters sharing a name at
the same scope and environment: \code{parameters} carries no uniqueness
constraint and \textbf{Add Parameter} does not refuse a duplicate. Before
it, which of them applied --- and therefore which default value, type,
required flag and \emph{sensitivity} an order was validated against ---
was whatever order the database happened to return the rows in, and could
differ between two page loads. Sensitivity is the sharp edge: it decides
whether the value is redacted everywhere downstream, so a pair of
duplicates that disagreed about it made redaction a question about row
order. Rule 4 makes duplicates predictable; it does not make them a good
idea, and a duplicate found at the same level should be deleted.
...
\subsection{Budgets}
\label{sec:cost-centre-budgets}

Every cost centre may carry a budget, set by \textbf{root} alone under
\textbf{Administration → Cost Centers → Budget}. It is opt-in: a cost
centre without one behaves exactly as it did before budgets existed. The
guard is deliberately narrower than the rest of that screen, which is
\code{admin} --- renaming a cost centre and deciding what the platform
refuses to provision are different powers.

A budget is an amount, a currency, a period and a behaviour:

\begin{description}
  \item[Total] One pot that never resets.
  \item[Monthly] Resets each calendar month.
  \item[Warn] The order goes through. The orderer is told at checkout,
        the approver sees it on the queue row, and the audit log records
        \code{order.budget\_warning}.
  \item[Block] The order is refused with HTTP 409. Root, and only root,
        may override a single order; that is audited as
        \code{order.budget\_overridden} and the orderer is still told the
        order went through over budget.
\end{description}

The check runs in \code{createPreparedOrder}, the one point both checkout
and approval traverse, so an order approved next week cannot spend a
budget that is already gone.

\paragraph{Two definitions that are stated rather than hidden.}
Both are consequences of what the data model actually records, and both
are surfaced in the UI rather than left for a reader of the code:

\begin{enumerate}
  \item \textbf{``Committed'' includes \code{pending}.} The costs report
        counts \code{provisioning} and \code{completed} --- what was
        actually built --- which is the right question for a spend report
        and the wrong one for a gate. An approval queue full of pending
        orders would each see budget left, because none of them exists
        yet, and then collectively blow it the moment they are approved.
  \item \textbf{``Monthly'' means orders \emph{placed} in the calendar
        month.} \code{product\_environments.price} records an amount and
        a currency and no billing period, so no honest run rate can be
        derived from it --- the costs report carries the same caveat for
        the same reason. A machine ordered in January
        and still running in June consumes budget in January only, and a
        long-lived estate therefore looks cheaper than it is. Giving a
        price a billing period is its own piece of work.
\end{enumerate}

Committed spend in other currencies is converted through
\code{exchange\_rates} with EUR as the rate base. An amount with no
stored rate is reported separately rather than folded into the total at a
rate that does not exist --- the same choice the costs report makes.

Attribution follows the costs report exactly: an order counts against its
own \code{cost\_center\_id} where it has one (\emph{Select} and
\emph{Overhead} modes) and against its project's in the default
\emph{Project} mode, where the order deliberately stores none.

\paragraph{What the check does, and what it does not.}
The verdict asks two questions, not one: is the budget already spent,
and would \emph{this} order take it over? Asking only the first would let
the very first order against an untouched budget be for any amount at
all. An order that lands exactly on the limit is inside it; the next one
is not.

Two figures are reported rather than guessed. Committed orders with no
recoverable price --- no snapshot, and the offering withdrawn since ---
are counted and shown, because their spend is missing from the total and
there is no honest number to add. Amounts in a currency with no stored
rate are reported separately. An \emph{incoming} order in an
unconvertible currency is refused by a \code{block} budget: with no rate
there is no way to show it fits, and the message names the missing rate
as the remedy.

Under concurrency the block is best-effort. The check and the insert are
separate operations, so two orders placed against one cost centre in the
same instant can both pass a check only one should have; the overspend is
bounded by a single order. Making it strict means serialising all
ordering against a cost centre, which is a throughput cost paid on every
order --- the trade-off is recorded in issue \#403.
...
\begin{longtable}{@{}L{4.5cm}L{7.5cm}l@{}}
  ...
  \env{SECRET\_ENCRYPTION\_KEY} & AES-256-GCM key for stored integration credentials — 64 hex characters. Treat as unrotatable once in use: there is no re-encryption path, so a changed key means re-entering every credential & No \\
  \env{SESSION\_TTL\_SECONDS}   & Ordinary login lifetime in seconds (default 8\,h, clamped to 30 days) & No \\
  \env{SESSION\_REMEMBER\_ME\_TTL\_SECONDS} & Lifetime with ``remember me'' ticked (default 30 days, same clamp) & No \\
  \env{TRUST\_PROXY}            & Trust \code{X-Forwarded-For}. Set only behind a proxy you control: it decides the per-IP login rate-limit bucket and the address shown in a session list. Unset records no address at all & No \\
  \env{DECOMMISSION\_SWEEP\_SECRET} & Shared secret for \code{POST /api/internal/decommission-sweep}, sent as \code{X-Sweep-Secret}. Blank disables the endpoint (503) & No \\
  \env{DRIFT\_REPORT\_SECRET}   & Shared secret for the scheduled drift loop, sent as \code{X-Drift-Secret}. Authorises \emph{both} \code{GET /api/internal/drift-targets} and \code{POST /api/internal/drift-report}. Blank disables both (503) & No \\
  \env{DEPLOYMENT\_WINDOW\_SWEEP\_SECRET} & Shared secret for \code{POST /api/internal/deployment-window-sweep} (\S\ref{sec:deployment-windows}), sent as \code{X-Sweep-Secret}. Blank disables the endpoint (503), and an order that reached \code{scheduled} then waits indefinitely & No \\
  \env{ALLOW\_INSECURE\_CI\_TRANSPORT} & Accept a plaintext \code{http} CI source off loopback. Every call to one carries a credential, so this is refused by default (\#329) & No \\
  \env{DEMO\_CI\_URL}           & CI endpoint the demo seed points its stack at; without it the seeded catalogue has nothing provisionable & No \\
  \env{ALLOW\_DEMO\_SEED\_IN\_PRODUCTION} & Permit \code{make db-seed-demo} with \code{NODE\_ENV=production}, which it otherwise refuses & No \\
  \bottomrule
...
\begin{longtable}{@{}lL{7.8cm}L{4.5cm}l@{}}
  ...
  DELETE & \code{/api/admin/cost-centers/\{id\}}                     & Delete cost center                      & root \\
  GET  & \code{/api/admin/cost-centers/budgets}                      & Every cost centre's budget state        & root \\
  GET  & \code{/api/admin/cost-centers/\{id\}/budget}                & Budget and committed spend              & root \\
  PUT  & \code{/api/admin/cost-centers/\{id\}/budget}                & Set or change the budget                & root \\
  DELETE & \code{/api/admin/cost-centers/\{id\}/budget}              & Remove the budget                       & root \\[6pt]
  ...
  POST & \code{/api/webhooks/bitbucket/pipeline}                     & Bitbucket pipeline status event         & public \\[6pt]

  \multicolumn{4}{@{}l}{\textit{Internal (scheduler-authenticated, shared secret in a header)}} \\[2pt]
  POST & \code{/api/internal/decommission-sweep}                     & Tear down elements past their expiry     & secret \\
  GET  & \code{/api/internal/drift-targets}                          & Work list for the drift pipeline         & secret \\
  POST & \code{/api/internal/drift-report}                           & Record what \code{plan -refresh-only} found & secret \\
  POST & \code{/api/internal/deployment-window-sweep}                 & Release orders whose window has opened   & secret \\
  POST & \code{/api/internal/holiday-refresh}                        & Pull the holiday feed into the cache     & secret \\[6pt]

  \multicolumn{4}{@{}l}{\textit{Deployment windows (root)}} \\[2pt]
  GET  & \code{/api/admin/deployment-windows}                         & The windows and the time zone            & root \\
  PUT  & \code{/api/admin/deployment-windows}                         & Replace the window set                   & root \\
  GET  & \code{/api/admin/holidays}                                   & Feed status and the cached dates         & root \\
  PATCH & \code{/api/admin/holidays}                                  & Set, preview or refresh the feed         & root \\
  PUT  & \code{/api/admin/holidays}                                   & Add or amend one date by hand            & root \\
  DELETE & \code{/api/admin/holidays}                                 & Remove one date                          & root \\
  POST & \code{/api/orders/\{id\}/deploy-now}                          & Deploy a scheduled order early           & root \\
  \bottomrule
  \caption{Complete REST API route reference}
  \label{tab:routes}
\end{longtable}

\paragraph{The internal routes.} These five have no user. A scheduler has no
session to present, so each is authenticated with a shared secret in a request
header, compared in constant time, and each returns \code{503} outright when its
secret is unset --- an unconfigured deployment must not be writable by an
anonymous caller.

Three secrets, not one, and \code{X-Sweep-Secret} carries a \emph{different}
one depending on the route --- configure the scheduler with the value that
belongs to the endpoint it calls:

\begin{itemize}
  \item \code{POST /api/internal/decommission-sweep} ---
        \code{X-Sweep-Secret}: \env{DECOMMISSION\_SWEEP\_SECRET}
  \item \code{POST /api/internal/deployment-window-sweep} and
        \code{POST /api/internal/holiday-refresh} --- \code{X-Sweep-Secret}:
        \env{DEPLOYMENT\_WINDOW\_SWEEP\_SECRET} for both
        (\S\ref{sec:deployment-windows}). One secret rather than two: same
        feature, same operator, and a secret per endpoint is a secret somebody
        forgets to set
  \item both drift routes --- \code{X-Drift-Secret}:
        \env{DRIFT\_REPORT\_SECRET}, which authorises the pair, so a deployment
        that sets it enables the whole loop and one that does not enables neither
\end{itemize}

The two drift routes are one cycle, run from a schedule in the
\code{infra-templates} project (\code{.ci/drift-report.gitlab-ci.yml}). The
\code{GET} returns the work list --- one entry per \emph{active} element, with
its base state name, its stack steps and the variables its apply used, because
\code{plan -refresh-only} needs the module and its inputs and a state file names
neither. The pipeline composes each step's state key as
\code{<base>-<stateSuffix>}, exactly as \code{generate\_stack.py} does, plans
each one, and \code{POST}s a single report. It is worth being plain that the
\code{GET} returns provisioning variables for the whole estate at once: not a
new class of exposure, since CI already receives exactly these when an element is
provisioned, but everything in one response --- hence the secret, and hence
nothing at all about inactive elements.

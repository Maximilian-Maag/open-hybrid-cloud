...
\begin{enumerate}
  ...
  \item The service fetches the order (returns \code{Err 400} if not
        ...
        JSON and POST it to each stack's orchestrator webhook URL.
...
\begin{itemize}
  ...
  \item \textbf{Upstream dependencies:} The optional \code{upstreamSuffix}
        field lets a step declare which preceding step it depends on.
        The orchestrator can use this to gate execution — e.g. DNS step
        waits for VM step outputs (IP address).
...
\code{upstreamSuffix} field in \code{PIPELINE\_STACK}. The orchestrator
passes the upstream step's state name as \code{VM\_STATE\_NAME} to the
downstream template, which can then read shared outputs via a
\code{terraform\_remote\_state} data source:
...
into a sequential GitLab child pipeline, then executes it. See
Section~\ref{sec:webhook} for how the generated pipeline resolves
inter-step state dependencies.

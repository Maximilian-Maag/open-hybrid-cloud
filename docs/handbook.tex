...
come from the \code{holidays} table.
...
\begin{longtable}{@{}lL{7.8cm}L{4.5cm}l@{}}
  ...
  POST & \code{/api/internal/deployment-window-sweep}                 & Release orders whose window has opened   & secret \\[6pt]
...
\paragraph{The internal routes.} These four have no user. A scheduler has no
...
\begin{itemize}
  ...
  \item \code{POST /api/internal/deployment-window-sweep} ---
        \code{X-Sweep-Secret}: \env{DEPLOYMENT\_WINDOW\_SWEEP\_SECRET}
        (\S\ref{sec:deployment-windows})

...
| FA-01.2 | Admins and Project Managers authenticate with email and password. Microsoft Entra ID (OIDC) SSO is optionally supported and configurable via environment variables. | Partially shipped — email/password is shipped (`apps/backend/src/app/api/auth/login/route.ts`). SSO is **not**: a callback handler existed but nothing on the frontend consumed it, no route ever started the flow, and it returned the session JWT in a query string. Removed in #139 rather than left advertising itself; see #250 for what a real implementation needs. |
...
| NFA-02.6 | The catalogue search must answer in bounded time, whatever the catalogue holds. | Bounded rather than indexed (#236). The predicate is a leading-wildcard `ILIKE` over a COALESCE of per-language subqueries, so no btree can serve it and every candidate row is evaluated in full. The match count therefore stops at `CATALOG_COUNT_CAP` (500) and reports `totalIsExact: false` above it; the page itself is bounded by `CATALOG_MAX_LIMIT`. **This is sized for a catalogue of hundreds to a few thousand products**, where every real search is counted exactly. An order of magnitude more needs a trigram (`pg_trgm` + GIN on a materialised column) or full-text index — the trade-offs, including that full-text would stop `kuber` matching `Kubernetes`, are written up in #236. |
...
| ID | Requirement | Traceability |
|----|-------------|--------------|
| NFA-03.1 | SSO authentication is performed via Microsoft Entra ID using the OpenID Connect Authorization Code Flow. | **Not shipped.** Was marked shipped against a callback handler that no route reached and no frontend consumed, with the `id_token` signature, `aud` and `iss` unverified and no `state`/`nonce`. Removed in #139; see #250. |
...
| NFA-05.2 | Local administrator accounts (Root and Admin) **must** hold a TOTP second factor (RFC 6238, SHA-1, 30 s window, 6 digits), compatible with standard authenticator apps. SSO accounts are covered by Entra ID MFA instead. | Shipped (#36) as opt-in and Root-only; widened and made mandatory in #197 on the owner's instruction. `lib/auth/totp.ts` verified against the RFC 4226/6238 published vectors; the requirement itself is enforced by `requireAuth`, which answers 403 `second_factor_required` on every route but enrolment until a factor is confirmed. Project Manager is excluded — it is the end-user role. The "SSO accounts are covered by Entra ID MFA instead" half no longer applies: SSO was removed in #139. The exemption itself keys on the account having no local password, not on `sso_sub`, so it still behaves correctly for rows an earlier deployment created. |
| NFA-05.2a | Administrators may register WebAuthn/FIDO2 credentials — hardware security keys and passkeys — as an alternative second factor. Either factor satisfies the requirement. | Shipped (#197 part 2) — `lib/services/webauthn.ts` over `@simplewebauthn/server`. Phishing-resistant where TOTP is not: the assertion is signed over the origin, so a lookalike domain cannot produce one that verifies. RP ID and origin are deployment configuration (`WEBAUTHN_RP_ID`, `WEBAUTHN_RP_ORIGIN`), validated at first use because getting them wrong works on localhost and breaks every key on the deployed instance. |
| NFA-05.2b | A WebAuthn ceremony challenge is server-generated and usable exactly once. | Shipped (#197 part 2) — the challenge is a row, claimed by the `DELETE … RETURNING` that reads it, so two racing requests cannot both spend it. A registration challenge cannot be redeemed as an authentication one; the kind is part of the claim. |

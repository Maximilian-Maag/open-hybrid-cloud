...
func (r *brandingRepo) Save(ctx context.Context, b *model.Branding) error {
	...
	if len(b.LogoData) > 0 {
		_, err = r.pool.Exec(ctx,
			`INSERT INTO branding(id,logo_data,logo_mime,primary_color,secondary_color) VALUES(1,$1,$2,$3,$4)
			...
			b.LogoData, b.LogoMime, b.PrimaryColor, b.SecondaryColor)
	} else {
		_, err = r.pool.Exec(ctx,
			`INSERT INTO branding(id,primary_color,secondary_color) VALUES(1,$1,$2)
             ON CONFLICT(id) DO UPDATE SET primary_color=EXCLUDED.primary_color, secondary_color=EXCLUDED.secondary_color`,
			b.PrimaryColor, b.SecondaryColor)
	}
	return err
}

...
func RunProductCascadeSmokeTest() {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO users(email,name,role) VALUES('smoke@example','Smoke','admin') RETURNING id`).Scan(&userID); err != nil {
	...
	// Insert infra element referencing the product and order
	...
	// Check whether order and infra rows still exist
	var cnt int
